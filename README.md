...
`just quality-precommit` 会先执行 Rust 与前端源码合规检查，再执行格式、Clippy、workspace check 与测试；`just quality-complete` 在其基础上执行 `cargo audit` 和 `cargo deny check`。提交前运行前者，完成 Rust 工作前运行后者。
