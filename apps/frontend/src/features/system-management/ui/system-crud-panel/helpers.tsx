...
export function displayField<T>(value: unknown, field: CrudField<T>, labels: DisplayLabels) {
  const special = displaySpecialField(value, field, labels);
  return special ?? displayTextField(value, field);
}

function displaySpecialField<T>(
  value: unknown,
  field: CrudField<T>,
...
function displayTextField<T>(value: unknown, field: CrudField<T>): ReactNode {
