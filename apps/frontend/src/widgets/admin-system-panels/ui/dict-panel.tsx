...
const DEFAULT_DATA_INPUT: DictDataInput = {
  dict_sort: 0,
  dict_label: '',
  dict_value: '',
  dict_type: '',
  ...
  return (
    <DashboardContent>
          ...
          <Stack direction="row" spacing={1}>
            ...
            {canRemove && (
              <Button
                ...
                onClick={() => setBatchDeleteTypeOpen(true)}
              >
                {t('common.delete')}
              </Button>
            )}
            {canAdd && (
              <AddButton onClick={() => setCreatingType(true)}>{t('actions.addDict')}</AddButton>
            )}
          </Stack>
        }
      />
        ...
        <Card>
          ...
          <Scrollbar>
            <Table sx={{ minWidth: 920 }}>
              <ManagementTableHead
                head={typeHead}
                rowCount={dictTypes.items.length}
                numSelected={selectedTypeIds.length}
                onSelectAllRows={canRemove ? toggleAllTypes : undefined}
              />
              <TableBody>
                {dictTypes.isLoading ? (
                  <TableLoadingRows head={loadingTypeHead} rows={typeTable.rowsPerPage} />
                ) : (
                  dictTypes.items.map((row) => (
                    <DictTypeRow
                      key={row.dict_id}
                      row={row}
                      selected={row.dict_type === activeType}
                      checked={selectedTypeIds.includes(row.dict_id)}
                      canRemove={canRemove}
                      onCheck={(id) => setSelectedTypeIds(toggle(selectedTypeIds, id))}
                ...
                <TableNoData
                  ...
                  notFound={!dictTypes.isLoading && dictTypes.items.length === 0}
                />
              </TableBody>
            </Table>
          </Scrollbar>
          <TablePaginationCustom
            page={typeTable.page}
            count={dictTypes.total}
            rowsPerPage={typeTable.rowsPerPage}
            onPageChange={typeTable.onChangePage}
            onRowsPerPageChange={typeTable.onChangeRowsPerPage}
          />
        </Card>
        <Card>
            ...
            <Stack direction="row" spacing={1}>
              ...
              {canRemove && (
                <Button
                  ...
                  onClick={() => setBatchDeleteDataOpen(true)}
                >
                  {t('common.delete')}
                </Button>
              )}
          ...
          <Scrollbar>
            <Table sx={{ minWidth: 1080 }}>
              <ManagementTableHead
                head={dataHead}
                rowCount={dictData.items.length}
                numSelected={selectedDataIds.length}
                onSelectAllRows={canRemove ? toggleAllData : undefined}
              />
              <TableBody>
                {dictData.isLoading ? (
                  <TableLoadingRows head={loadingDataHead} rows={dataTable.rowsPerPage} />
                ) : (
                  dictData.items.map((row) => (
                    <DictDataRow
                      key={row.dict_code}
                      row={row}
                      selected={selectedDataIds.includes(row.dict_code)}
                      canRemove={canRemove}
                      onCheck={(id) => setSelectedDataIds(toggle(selectedDataIds, id))}
                ...
                <TableNoData
                  title={t('common.noData')}
                  notFound={!dictData.isLoading && dictData.items.length === 0}
                />
              </TableBody>
            </Table>
          </Scrollbar>
          <TablePaginationCustom
            page={dataTable.page}
            count={dictData.total}
            rowsPerPage={dataTable.rowsPerPage}
            onPageChange={dataTable.onChangePage}
            onRowsPerPageChange={dataTable.onChangeRowsPerPage}
          />
        </Card>
      </Stack>
      <DictTypeDialog
      ...
      <ConfirmDialog
        ...
        onClose={() => setBatchDeleteTypeOpen(false)}
        title={t('common.delete')}
        content={t('dialogs.deleteContent', { name: String(selectedTypeIds.length) })}
        cancelText={t('common.cancel')}
        action={
          <Button variant="contained" color="error" onClick={confirmBatchDeleteTypes}>
            {t('common.delete')}
          </Button>
        }
      />
      <ConfirmDialog
        ...
        onClose={() => setBatchDeleteDataOpen(false)}
        title={t('common.delete')}
        content={t('dialogs.deleteContent', { name: String(selectedDataIds.length) })}
        cancelText={t('common.cancel')}
        action={
          <Button variant="contained" color="error" onClick={confirmBatchDeleteData}>
            {t('common.delete')}
          </Button>
        }
      />
      <ConfirmDialog
        ...
        cancelText={t('common.cancel')}
        action={
          <Button variant="contained" color="error" onClick={confirmDeleteType}>
            {t('common.delete')}
          </Button>
        }
      />
      <ConfirmDialog
        ...
        cancelText={t('common.cancel')}
        action={
          <Button variant="contained" color="error" onClick={confirmDeleteData}>
            {t('common.delete')}
          </Button>
        }
      />
...
function DictTypeFilters({
  filters,
  onChange,
}: {
  filters: typeof DEFAULT_TYPE_FILTERS;
  onChange: (filters: typeof DEFAULT_TYPE_FILTERS) => void;
}) {
  const { t } = useTranslate('admin');
  const write = (key: keyof typeof DEFAULT_TYPE_FILTERS, value: string) =>
...
function DictDataFilters({
  filters,
  onChange,
}: {
  filters: typeof DEFAULT_DATA_FILTERS;
  onChange: (filters: typeof DEFAULT_DATA_FILTERS) => void;
}) {
  const { t } = useTranslate('admin');
  const write = (key: keyof typeof DEFAULT_DATA_FILTERS, value: string) =>
    onChange({ ...filters, [key]: value });
  return (
    ...
    <TableRow hover selected={selected} onClick={() => onSelect(row)}>
      ...
      <TableCell align="right">
        <RowActions
          ...
          editDisabled={!useHasPermission('system:dict:edit')}
          deleteDisabled={!useHasPermission('system:dict:remove')}
        />
      </TableCell>
    </TableRow>
  );
...
function dictTypeHead(t: ReturnType<typeof useTranslate>['t']): TableHeadCellProps[] {
  return [
...
function dictDataHead(t: ReturnType<typeof useTranslate>['t']): TableHeadCellProps[] {
  return [
