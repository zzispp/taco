...
type TreeSelectDialogProps = Readonly<{
  open: boolean;
  nodes: TreeSelectDialogNode[];
  value: string;
  rootLabel: string;
...
type TreeDialogContentProps = Pick<
  TreeSelectDialogProps,
  'nodes' | 'value' | 'rootLabel' | 'rootValue' | 'allowRoot' | 'onChange'
> &
  Readonly<{
    allIds: string[];
    expandedIds: string[];
...
function TreeDialogContent({
  ...
  return (
    <>
      ...
      <List disablePadding>
        {allowRoot && (
          <ListItemButton dense selected={value === rootValue} onClick={() => onChange(rootValue)}>
            <Box sx={{ width: 34 }} />
...
type TreeSelectRowProps = Readonly<{
...
function TreeSelectRow({ node, level, value, expanded, onToggle, onChange }: TreeSelectRowProps) {
  ...
  return (
    <>
      ...
      >
        <TreeNodeToggle node={node} open={open} onToggle={onToggle} />
        <ListItemText primary={node.label} />
      </ListItemButton>
      {hasChildren && (
        <TreeNodeChildren {...{ node, level, value, expanded, onToggle, onChange, open }} />
      )}
    </>
  );
}
...
function TreeNodeToggle({
  ...
  open,
  onToggle,
}: Pick<TreeSelectRowProps, 'node' | 'onToggle'> & Readonly<{ open: boolean }>) {
  if (node.children.length === 0) return <Box sx={{ width: 34 }} />;
  return (
    <IconButton
      size="small"
      onClick={(event) => {
...
type TreeNodeChildrenProps = TreeSelectRowProps & Readonly<{ open: boolean }>;
