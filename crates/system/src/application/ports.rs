...
pub enum SystemExportKind {
    ...
    DictTypes(DictTypeListFilter),
    DictData(DictDataListFilter),
    Configs(ConfigListFilter),
}
...
pub enum SystemExportBatch {
    Posts(Vec<Post>),
    DictTypes(Vec<DictType>),
    DictData(Vec<DictData>),
    Configs(Vec<ConfigItem>),
}

impl SystemExportBatch {
    pub fn len(&self) -> usize {
        match self {
            Self::Posts(items) => items.len(),
            Self::DictTypes(items) => items.len(),
            Self::DictData(items) => items.len(),
            Self::Configs(items) => items.len(),
        }
    }
...
pub trait SystemExportSink: Send {
