...
pub fn create_router(state: SystemApiState) -> Result<Router, audit_contract::EndpointSpecError> {
    Ok(structure_routes()?.merge(data_routes()?).with_state(state))
}

fn structure_routes() -> Result<Router<SystemApiState>, audit_contract::EndpointSpecError> {
...
fn data_routes() -> Result<Router<SystemApiState>, audit_contract::EndpointSpecError> {
