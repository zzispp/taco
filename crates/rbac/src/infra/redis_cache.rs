...
impl RbacCache for RedisRbacCache {
    async fn write_snapshot(&self, snapshot: &PermissionSnapshot) -> RbacResult<()> {
        ...
        self.observer
            .observe(
                ...
                "rbac_cache_write_snapshot",
                connection.set(self.snapshot_key(), snapshot_json),
            )
            .await
            .map_err(redis_error)
    }
    ...
    async fn read_snapshot(&self) -> RbacResult<PermissionSnapshot> {
        ...
        let value: Option<String> = self
            .observer
            .observe(InfrastructureDependency::Redis, "rbac_cache_read_snapshot", connection.get(self.snapshot_key()))
            .await
            .map_err(redis_error)?;
