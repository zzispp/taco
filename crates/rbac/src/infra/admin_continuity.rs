...
const ENABLED_ADMIN_EXISTS_QUERY: &str = r#"
    SELECT EXISTS(
        SELECT 1
...
pub(crate) async fn acquire_admin_continuity_lock(transaction: &mut Transaction<'_, Postgres>) -> StorageResult<()> {
...
pub(crate) async fn ensure_enabled_admin_remains(transaction: &mut Transaction<'_, Postgres>) -> StorageResult<()> {
    let has_enabled_admin = query_scalar::<_, bool>(ENABLED_ADMIN_EXISTS_QUERY)
        .bind(ADMIN_ROLE_KEY)
        .bind(STATUS_NORMAL)
